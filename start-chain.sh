...
add_genesis_account() {
    ...
    if [ ! "$(jq '.app_state.auth.accounts | any(.address == $address)' "$AGORIC_HOME/config/genesis.json" --arg address "$address" 2>&1)" == "true" ]; then
...
create_wallet() {
    ...
    if ! agd keys show "$key" --address --home "$AGORIC_HOME" --keyring-backend "test" >/dev/null 2>&1; then
        echo "$recovery_key" |
            agd keys add "$key" \
                --home="$AGORIC_HOME" --keyring-backend=test \
...
generate_genesis_file_from_genesis_transaction() {
    if [ ! "$(jq '.app_state.genutil.gen_txs | length > 1' "$AGORIC_HOME/config/genesis.json" 2>&1)" == "true" ]; then
